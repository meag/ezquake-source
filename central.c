...
#ifndef SERVER_ONLY
// Have set this to read-only to stop this accidentally being set to another site by someone else's config
...
#ifndef SERVER_ONLY
static void Web_SubmitRequestFormClient(const char* url, struct curl_httppost* first_form_ptr, struct curl_httppost* last_form_ptr, web_response_func_t callback, const char* requestId, void* internal_data, const char* authkey)
{
	...
	if (code != CURL_FORMADD_OK) {
		...
		Con_Printf("Failed to generate form (1: %u)\n", code);
		return;
	}
...
void Central_FindChallengeResponse(const char* token, const char* challengeText)
{
	...
	Web_SubmitRequestFormClient(url, first_form_ptr, last_form_ptr, Auth_FindChallengeResponse, challengeText, NULL, token);
}
#endif
